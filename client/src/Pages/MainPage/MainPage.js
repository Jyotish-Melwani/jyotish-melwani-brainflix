...
class MainPage extends Component {
  ...
  axiosPost = (id, newComment) => {
    return axios
      .post(`${API_URL}videos/${id}/comments`, newComment)
      .then((response) => {
        this.axiosGet(id);
      })
  ...
  axiosDelete = (id, commentsId) => {
    return axios
      .delete(`${API_URL}videos/${id}/comments/${commentsId}`)
      .then((response) => {
        this.axiosGet(id);
      })
      .catch((err) => console.log("Error", err));
  ...
  componentDidMount() {
    axios
      ...
      .then((response) => {
        if (
          (this.state.currentPath === null || this.state.currentId === null) &&
      ...
      .catch((error) => {
        console.log("ERROR! Third Axios Inside!", error);
      });
  }
  ...
  render() {
    if (
      ...
      return <main className="load-screen">You forgot to run node on server.js</main>;
    }
    return (
      <div className="App">
        <Hero
          content={this.state.selectedData.image}
          source={this.state.selectedData.video}
          key={this.state.selectedData.id}
        />
        <div className="half-page">
          <div className="half-page-left">
            <VideoInfo
              content={this.state.selectedData}
              handleOnClick={this.handleOnClick}
            />
            <Comments
              selectedVideo={this.state.selectedData}
              {...this.props}
